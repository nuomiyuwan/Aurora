...
build_directory=$(mktemp -d "${TMPDIR:-/tmp}/aurora-ffmpeg-build.XXXXXX")
...
(
  ...
  ./configure \
    ...
    --target-os=darwin \
    --cc='xcrun clang' \
    --extra-cflags="-mmacosx-version-min=12.0 -I$libvpx_install_directory/include" \
    --extra-ldflags="-mmacosx-version-min=12.0 -L$libvpx_install_directory/lib" \
    --disable-autodetect \
    --disable-doc \
    --disable-debug \
    --disable-ffplay \
...
macos_binary_directory="$project_directory/resources/bin/mac-arm64"
...
echo "Installed FFmpeg $ffmpeg_tag with libvpx $libvpx_tag for macOS arm64."
shasum -a 256 \
  "$macos_binary_directory/ffmpeg" \
  "$macos_binary_directory/ffprobe"
